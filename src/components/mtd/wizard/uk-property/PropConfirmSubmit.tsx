...
interface PropConfirmSubmitProps {
  state: MtdWizardState;
  onSubmit: () => Promise<void>;
  onBack: () => void;
}

export function PropConfirmSubmit({
  state,
  onSubmit,
  onBack,
}: PropConfirmSubmitProps) {
  ...
  const totalExpenses = state.useConsolidatedExpenses
    ? state.ukPropertyData?.expenses?.consolidatedExpenses || 0
    : UK_PROPERTY_EXPENSE_CATEGORIES.reduce(
        (sum, cat) =>
          sum + ((state.ukPropertyData?.expenses as any)?.[cat.key] || 0),
        0
      );
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">
          You're about to submit your quarterly property update to HMRC for{' '}
        ...
        <div className="space-y-3">
          <div className="flex justify-between py-2 border-b border-gray-100">
            ...
            <span className="text-sm font-medium text-gray-900">
              {state.businessName || 'UK Property'}
            </span>
          </div>
          ...
          <div className="flex justify-between py-2 border-b border-gray-100">
            <span className="text-sm text-gray-600">Total property income</span>
          ...
          <div className="flex justify-between py-2 border-b border-gray-100">
            <span className="text-sm text-gray-600">Total property expenses</span>
            <span className="text-sm font-medium text-gray-900">
              {formatCurrency(totalExpenses)}
            </span>
          </div>

          {income.taxDeducted && income.taxDeducted > 0 && (
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-sm text-gray-600">Tax already deducted</span>
              <span className="text-sm font-medium text-gray-900">
                {formatCurrency(income.taxDeducted)}
              </span>
            </div>
          )}
