...
function getStaticDeadlines(taxYear: string): Deadline[] {
  ...
  quarters.forEach((q, i) => {
    ...
    if (daysLeft > 0) {
      deadlines.push({
        name: `Q${i + 1} Submission`,
        date: formatDeadlineDate(q.deadline),
        daysLeft,
        urgent: daysLeft <= 30,
      });
    }
  ...
  // SA filing deadline (31 Jan following end of tax year)
  const saDate = `${startYear + 2}-01-31`;
  const saDays = getDaysUntil(saDate);
  if (saDays > 0 && saDays <= 365) {
    deadlines.push({
      name: 'SA Filing Deadline',
      date: formatDeadlineDate(saDate),
      daysLeft: saDays,
      urgent: saDays <= 30,
    });
  }

  // Payment on Account (31 Jan)
...
export function UpcomingDeadlines({ hasHmrcConnection, taxYear: taxYearProp }: UpcomingDeadlinesProps) {
  ...
  const staticDeadlines = useMemo(() => getStaticDeadlines(taxYear), [taxYear]);
  const [deadlines, setDeadlines] = useState<Deadline[]>(staticDeadlines);
  const [loading, setLoading] = useState(hasHmrcConnection);

  useEffect(() => {
    if (!hasHmrcConnection) {
      setDeadlines(staticDeadlines);
      setLoading(false);
      return;
    }
        ...
        if (openObligations.length > 0) {
          const mapped: Deadline[] = openObligations.map((o) => {
            ...
            const qIdx = quarters.findIndex(
              (q) => q.start === o.periodStartDate && q.end === o.periodEndDate
            );
            const daysLeft = getDaysUntil(o.dueDate);
            return {
              name: `Q${qIdx + 1} Submission`,
          ...
          // Add payment on account if close
          const startYear = parseInt(taxYear.split('-')[0]);
          const poaDate = `${startYear + 1}-01-31`;
          const poaDays = getDaysUntil(poaDate);
          if (poaDays > 0 && poaDays <= 180) {
          ...
          // All obligations fulfilled — fall back to static deadlines
          setDeadlines(staticDeadlines);
        }
      } catch {
        setDeadlines(staticDeadlines);
    ...
    fetchDeadlines();
  }, [hasHmrcConnection, taxYear, staticDeadlines]);
      ...
      >
        <Card>
          <CardContent>
            ...
            <p className="text-sm text-gray-400">All deadlines passed for this tax year.</p>
          </CardContent>
        </Card>
      </motion.div>
    );
